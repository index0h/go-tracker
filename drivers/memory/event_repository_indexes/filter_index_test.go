...
func filterIndexGenerateEvent(filters map[string]string) *entities.Event {
	...
	eventA, _ := entities.NewEvent(uuidMaker.Generate(), true, map[string]string{}, filters)
...
func filterIndexGenerateVisit(data map[string]string) *entities.Visit {
	...
	visit, _ := entities.NewVisit(uuidMaker.Generate(), int64(0), uuidMaker.Generate(), "", data, []string{})
	return visit
}
