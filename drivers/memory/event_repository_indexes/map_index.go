...
type MapIndex struct {
	...
	events map[common.UUID]*entities.Event
}

func NewMapIndex() *MapIndex {
	return &MapIndex{events: make(map[common.UUID]*entities.Event)}
}

func (index *MapIndex) Refresh(events []*entities.Event) {
	tmpStorage := make(map[common.UUID]*entities.Event)
...
func (index *MapIndex) FindByID(uuid common.UUID) (result *entities.Event, err error) {
	if common.IsUUIDEmpty(uuid) {
